...
app.get('/', async (req, res) => {
    ...
    // Получаем URL аватарки (если её нет — ставим заглушку)
    const avatarUrl = me.avatar 
        ? pb.files.getUrl(me, me.avatar) 
    ...
    try {
        ...
        <style>
            ...
            #editModal, #profileModal { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); justify-content: center; align-items: center; z-index: 100; }
        ...
        <div class="container">
            <div class="card profile-section">
                ...
                <div style="flex-grow: 1;">
                    <h2 style="margin:0;">Привет, ${me.email.split('@')[0]}!</h2>
                    <span class="badge" style="background:#6366f1;">${me.role}</span>
                </div>
                <div style="display:flex; gap:10px;">
                    <button onclick="document.getElementById('profileModal').style.display='flex'" class="btn btn-primary">⚙️ Настройки</button>
            ...
            <div id="profileModal">
                <div class="modal-content">
                    ...
                    <form action="/update-profile" method="POST" enctype="multipart/form-data">
                        <label>Новая аватарка:</label><br>
                        <input type="file" name="avatar" accept="image/*" style="margin:10px 0; width:100%;"><br><br>
                        
                        <label>Сменить пароль:</label><br>
                        <input type="password" name="password" placeholder="Новый пароль" style="margin:10px 0; width:100%;">
                        <input type="password" name="passwordConfirm" placeholder="Подтвердите пароль" style="margin:10px 0; width:100%;">
                        
                        <div style="display:flex; gap:10px; margin-top:20px;">
                            <button type="submit" class="btn btn-success" style="flex:1;">Сохранить</button>
                            <button type="button" onclick="document.getElementById('profileModal').style.display='none'" class="btn btn-danger" style="flex:1;">Закрыть</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <form method="POST" action="/purchase">
                    <table>
                        <thead>
                            <tr>${isUser ? '<th>🛒</th>' : ''} <th>Название</th> <th>Цена</th> ${!isUser ? '<th>Состояние</th> <th>Действия</th>' : ''}</tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                ${!isUser ? `
                                    <td><span class="badge" style="background:${i.work === 'working' ? '#059669' : '#dc2626'}">${i.work}</span></td>
                                    <td>
                                        <a href="/toggle-status/${i.id}/${i.work}" class="btn" style="background:#475569; font-size:0.8em;">Переключить</a>
                                        ${isAdmin ? `<a href="/del-item/${i.id}" style="margin-left:10px;">🗑️</a>` : ''}
                                    </td>
                                ` : ''}
                            </tr>`).join('')}
                        </tbody>
                    </table>
                    ${isUser && availableItems.length > 0 ? `<button class="btn btn-success" style="width:100%; margin-top:20px; padding:15px;">ОПЛАТИТЬ</button>` : ''}
                </form>
            </div>
        </div>
        `;
...
app.post('/update-profile', upload.single('avatar'), async (req, res) => {
    try {
        ...
        // Если пароль менялся — нужно перелогиниться, но для простоты просто редиректим
        res.redirect('/');
    } catch (e) {
        res.status(500).send("Ошибка обновления: " + e.message);
    }
});
...
app.listen(PORT, '0.0.0.0', () => console.log(`🚀 SYSTEM READY ON PORT ${PORT}`));
