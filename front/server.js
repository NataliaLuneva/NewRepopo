    ...
    try {
        ...
        <div class="container">
            ...
            <div id="profileModal">
                <div class="modal-content">
                    ...
                    <form action="/update-profile" method="POST" enctype="multipart/form-data">
                        ...
                        <label>Новый пароль (мин. 8 симв.):</label><br>
            ...
            <div class="card">
                ...
                <table>
                    ${allUsers.map(u => `<tr><td>${u.email}</td><td><span class="badge">${u.role}</span></td><td style="text-align:right;"><a href="/del-user/${u.id}" style="color:#f43f5e; text-decoration:none;">❌</a></td></tr>`).join('')}
                </table>
            </div>
            ` : ''}
                        ...
                        <tbody>
                            ...
                            <tr class="item-row">
                                ...
                                ${!isUser ? `
                                    ...
                                    <td><a href="/toggle-status/${i.id}/${i.work}" class="btn" style="background:#475569; font-size:0.8em;">Переключить</a> ${isAdmin ? `<a href="/del-item/${i.id}" style="margin-left:10px;">🗑️</a>` : ''}</td>
                                ` : ''}
                            </tr>`).join('')}
                        </tbody>
                    </table>
                    ${isUser && availableItems.length > 0 ? `<button class="btn btn-success" style="width:100%; margin-top:20px; padding:15px;">КУПИТЬ</button>` : ''}
                </form>
            </div>
        </div>
...
app.post('/update-profile', upload.single('avatar'), async (req, res) => {
    try {
        ...
        // 1. Проверяем пароль (добавляем, только если поле не пустое)
        if (req.body.password && req.body.password.trim() !== "") {
            if (req.body.password.length < 8) {
                return res.status(400).send("Ошибка: Пароль должен быть не менее 8 символов!");
            }
            if (req.body.password !== req.body.passwordConfirm) {
                return res.status(400).send("Ошибка: Пароли не совпадают!");
            }
        ...
        // 2. Проверяем аватарку (добавляем, только если файл загружен)
        if (req.file) {
        ...
        // 3. Если ничего не введено — просто возвращаемся назад
        if (!hasData) {
            return res.redirect('/');
        }
        ...
        // Обновляем сессию, чтобы подтянулись новые данные (особенно имя файла аватарки)
        await pb.collection('users').authRefresh();
        
        res.redirect('/');
    } catch (e) {
        console.error("ДЕТАЛИ ОШИБКИ:", e.data);
        res.status(500).send(`Ошибка обновления: ${e.message}. Причина: ${JSON.stringify(e.data?.data || e.data)}`);
    }
});
